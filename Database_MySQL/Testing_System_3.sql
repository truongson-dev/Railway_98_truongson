-- ======================TẠO DATABASE==================================
DROP DATABASE Testing_System_3;
...
-- SubQuery: nhiều SELECT,.. lồng nhau trong một câu

SELECT * FROM`Account` 
WHERE 		LENGTH(Fullname) = (SELECT MAX(LENGTH(Fullname)) FROM `Account`) 
ORDER BY 	Fullname DESC;

WITH cte_maxLengFullname AS (
SELECT max(length(a1.fullname)) maxFullname FROM account a1 )
SELECT * FROM Account  a
WHERE length(a.FullName) = (SELECT maxFullname FROM cte_maxLengFullname);
...
-- Question 6: Lấy ra tên group đã tham gia trước ngày 2024-01-20 
SELECT GroupName FROM `Group` WHERE CreateDate <'2024-01-20';

-- Question 7: Lấy ra ID của question có >= 2câu trả lời 
SELECT a.QuestionID, count(a.QuestionID) AS SoLuong
FROM Answer a
GROUP BY a.QuestionID 
HAVING COUNT(a.AnswerID) >= 2;

-- Question 8: Lấy ra các mã đề thi có thời gian thi >= 60 phút và được tạo trước ngày 2024-02-10
SELECT `Code` FROM Exam 
WHERE Duration >= 60 AND CreateDate < '2024-02-10';
...
-- Question 11: Lấy ra nhân viên có tên bắt đầu bằng chữ "n" và kết thúc bằng chữ "a" 
SELECT 		Fullname 
FROM 		`Account`

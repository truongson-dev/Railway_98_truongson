...
discountedElectronics.forEach((item) => {
    console.log(`  ${item.name}: $${item.originalPrice} → $${item.discountedPrice.toFixed(2)}`);
});
// Tính tổng giá sau giảm
const totalDiscount = discountedElectronics.reduce((sum, item) => sum + item.discountedPrice, 0);
